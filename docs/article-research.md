...
葛飾区・東京都・東京商工会議所・東京都中小企業振興公社・日本政策金融公庫を検索対象へ追加しました。
引用資料が2件未満の場合のみ1回追加調査し、地域の情報とWeb制作一般の情報を分けて調べます。初回の資料も引き継ぎ、同じURLは重複カウントしません。調査全体では最大6ツール呼び出し・最大8000出力トークンになります。APIエラーや不完全な応答は自動再試行しません。
