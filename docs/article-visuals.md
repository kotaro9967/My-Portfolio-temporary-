...
手動の Generate article draft と週次の Weekly article drafts は、記事本文の品質チェック後に Playwright / Chromium で3枚撮影します。
...
4. 週次処理も同じ画像生成を使い、既存のレビュー通知を維持します。画像のアップロードに失敗した記事は保存せず、週次通知は要確認になります。
...
画像アップロードに失敗した場合、画像なしの記事を完成扱いにしません。記事生成のOpenAI費用は既に発生している場合があります。途中までアップロードした画像が残ることがあります。無条件リトライや自動削除は行いません。
